...
my %clib = (
    ...
    map( {
            my $u = uc;
                "is$_" => "is$u"
        } qw/alnum alpha cntrl digit graph lower print punct space upper xdigit/ ),
    map( {
            my $u = uc;
                "to$_" => "to$u"
        } qw/lower upper/ ),
...
print STDERR "Found " . ( keys %functions ) . " functions, " . ( keys %macros ) . " macros, " . ( keys %variables ) . " variables, " . ( keys %constants ) . " constants and " . ( keys %types ) . " types\n";

my $len = 78;

sub output {
    my ( $data, $type, $indent ) = @_;
    my $fh = *STDOUT;
    my $head = "syn keyword xs$type";
    my $line = $head;
    for ( sort keys %$data ) {
        if ( length() + length($line) + 1 >= $len ) {
            $line = "    " . $line if $type =~ /Private|Superseded/;
            print $fh "$line\n";
            $line = $head;
        }
        $line .= " $_";
...
output \%superseded, 'Superseded' => $vim, 1;
print $vim "endif\n";

print $vim "if exists(\"xs_not_core\") && xs_not_core\n";
output \%private, 'Private' => $vim, 1;
print $vim "endif\n";
